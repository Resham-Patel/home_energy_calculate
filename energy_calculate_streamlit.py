...
#     home_type = st.selectbox("Choose type of House:", ["Flat", "Tenament"])
#     rooms = st.selectbox("Choose room type:", ["1BHK", "2BHK", "3BHK"]).lower()
...
#         ac_energy = num_ac * 3 if has_ac == "Yes" else 0
#         fridge_energy = 4 if has_fridge == "Yes" else 0
#         washing_energy = 2 if has_washing == "Yes" else 0
...
#             breakdown_data["Base Power (kW)"].append(4.0)
#             breakdown_data["Base Consumption (kWh)"].append(4)
#             breakdown_data["Day-Adjusted Consumption (kWh)"].append(4)
...
#             st.metric(f"{selected_day} Consumption", f"{cal_energy:.1f} kWh")
#         with col2:
#             st.metric("Weekly Avg (estimated)", f"{cal_energy * 7 / day_multiplier:.1f} kWh")
#         with col3:
#             st.metric("Monthly Avg (estimated)", f"{cal_energy * 30 / day_multiplier:.1f} kWh")

#         st.subheader("💡 Energy Saving Tips")
#         if selected_day in ["Saturday", "Sunday"]:
#             st.write("- Weekend usage is usually higher—try using energy-intensive appliances during off-peak hours.")
#             st.write("- Use natural light during daytime hours.")
#         else:
#             st.write("- Turn off fans/lights before leaving home.")
#             st.write("- Delay washing/dishwashing to late evening hours for efficiency.")

#         st.write("**General Tips:**")
#         tips = [
#             "Use LED bulbs instead of incandescent to save energy.",
#             "Set AC to 24°C or higher for better efficiency.",
#             "Unplug chargers and appliances when not in use.",
#             "Keep appliances well-maintained to ensure optimal performance."
#         ]
#         for tip in tips:
#             st.write(f"• {tip}")

#     else:
#         st.error("Please fill in all required fields including day selection before calculating.")
...
# Page header
st.title("⚡ Energy Consumption Calculator")
st.markdown("Track your daily energy usage and build a smart weekly summary 📅")

st.markdown("---")

# Dual column layout for inputs
...
with left_col:
    ...
    home_type = st.selectbox("Type of House", ["Flat", "Tenament"])
    rooms = st.selectbox("Room Configuration", ["1BHK", "2BHK", "3BHK"]).lower()

with right_col:
    st.subheader("📅 Day & Appliances")

    selected_day = st.selectbox("Select Day", days)

    has_ac = st.radio("Do you have an AC?", ["Yes", "No"])
    num_ac = st.number_input("Number of ACs", min_value=1, max_value=10, value=1) if has_ac == "Yes" else 0
    has_fridge = st.radio("Do you have a fridge?", ["Yes", "No"])
    has_washing = st.radio("Do you have a washing machine?", ["Yes", "No"])

st.markdown("### 🛠️ Fill in the form and hit the button below")
submit = st.button("💾 Save Today's Data", type="primary")

# Day multipliers
day_multipliers = {
    "Monday": 1.0, "Tuesday": 1.0, "Wednesday": 1.0, "Thursday": 1.0,
    "Friday": 1.1, "Saturday": 1.3, "Sunday": 1.2
}

# Handle submission
if submit and name and age and area and city:
    fans = lights = 3 if rooms in ["2bhk", "3bhk"] else 2
    base_energy = (fans * 0.4) + (lights * 0.8)
    ac_energy = num_ac * 3 if has_ac == "Yes" else 0
    fridge_energy = 4 if has_fridge == "Yes" else 0
    washing_energy = 2 if has_washing == "Yes" else 0
    multiplier = day_multipliers[selected_day]
    total_energy = ((base_energy + ac_energy + washing_energy) * multiplier) + fridge_energy
    ...
    st.session_state.weekly_data[selected_day] = {
        ...
        "Total (kWh)": round(total_energy, 2)
    }

    st.success(f"✅ Energy data for **{selected_day}** has been saved!")

# Weekly summary
if st.session_state.weekly_data:
    st.markdown("---")
    st.header("📊 Weekly Energy Summary")

    df_week = pd.DataFrame.from_dict(st.session_state.weekly_data, orient="index")
    st.dataframe(df_week, use_container_width=True)

    st.bar_chart(df_week["Total (kWh)"])

# Footer
st.markdown("---")
st.markdown(
    """
    *Note: Energy estimates are based on standard appliance consumption.
    Actual values may vary based on usage habits and device efficiency.*
    """
)